...
def build_prompt(draw: Draw) -> str:
    ...
    lines = [
        ...
        *([f"主题偏向：{draw.theme}。"] if draw.theme else []),
        ...
        f"- 作品适配：{realm['fit']}。",
        "",
        "核心模型：法门 = 天位(动力因：为何成力/何因增强/何时耗尽/失控何状/如何度量，可多项式组合) x 地位(媒介/承载方式，可复合) x 人位(目的/作用，可多目标)。",
    ]
    if draw.diagnostics:
    ...
    lines.extend(
        [
            "",
            "天位动力因判据：",
            "- 天位必须先回答：此要素为何成力、何因增强、何时耗尽、失控扭成何状、修士如何度量。随后才写特效、用途、来历或权限。",
            "- 若解释只能说明法术主题、宗门权限、权柄名、侦测/攻击功能或合法性，而不能说明增强、耗尽、失控和度量，就不是合格天位解释。",
            "- 例如不要把「罚」写成惩罚权限，要写成应有后果尚未落下的反压；不要把「照」写成探照功能，要写成遮掩与真实之间差额释放的明证之力。",
            "",
            "天位/动力因：",
    ...
    lines.extend(
        [
            ...
            "- 五行是低中阶修士和百艺体系最常用的公共语法，因为它能把药性、材性、脉性、阵性、器性等偏性整理成可教学、可交易、可诊断、可配伍的生克旺衰关系；但五行不是唯一真理，也不能替代本次天位的动力因、增强、衰竭、失控和度量。",
            "- 输出重心必须符合修真小说世界观设定的比重：天位动力因与复合结构约占 45%-55%，施展/效果/代价/境界约占 25%-35%，诸名约占 10%-15%，设定集正文负责收束。不要让命名解释占据开头或最长篇幅。",
            "- 先立设定总览、核心三位、天位动力因和复合结构，再写诸名。名称是世界内流传层，不是正文主结构。",
            ...
            "- 对每个天位分别解释为何成力、何因增强、何时耗尽、失控何状和如何度量；不要把天位当作装饰词、主题、权限、特效名或合法性说明。",
            "- 不要把「罚」写成惩罚权限，不要把「照」写成探照功能，不要把火、水、雷、名、时序等写成主题标签；必须写成能供能、增长、耗尽、失控和度量的动力因。",
            ...
            *(
            ...
            "2. 核心三位",
            "3. 天位动力因（逐天位写：为何成力、何因增强、何时耗尽、失控何状、如何度量；先写动力因，再写效果）",
            "4. 复合结构与纯化取舍",
            "5. 地位媒介与施展/修炼方式",
            *(
...
def format_element(item: ElementDraw) -> str:
    ...
    if item.role.startswith("天位"):
        structure_text = (
            f"复合结构角色：{item.structure_role}；"
            "动力因检查：先说明为何成力、何因增强、何时耗尽、失控何状、如何度量，不得只写主题/权限/特效；"
            f"天位质量 {item.quality:.2f}「{item.quality_label}」；"
        )
